...
const modGuildId = process.env.MOD_GUILD;
const modAcceptedChannel = process.env.MOD_ACCEPT_CHANNEL;
...
if (!modGuildId) throw new Error("Moderator discord not defined in .env"); // guild id for modcord
if (!modAcceptedChannel) throw new Error("Moderator accepts channel not defined in .env"); // channel id for the text channel to post accepted appeals
...
const COMMAND_startvote = async (interaction: ChatInputCommandInteraction) => {
  ...
  if (channel && (channel instanceof TextChannel || channel instanceof ThreadChannel)) {
    try {
      ...
      let appealCreator: string | null = null; // Variable to store the appeal creator's ID
        ...
        if (message.author.id === ticketBotId) {
          await Promise.all(message.embeds.map(async (embed, index) => {
            if (index === 1) {
              ...
              // Extract user mentions from the message and store the first one as the appeal creator
              const userMentions = message.content.match(/<@!?\d+>/g);
              if (userMentions && userMentions.length > 0) {
                appealCreator = userMentions[0].replace(/[<@!>]/g, ''); // Extract user ID from mention
              }
            } else {
              interaction.reply({
      ...
      const voteMessage = await voteChannel.send({
        ...
        embeds: [{
          description: `${embedDescriptions.join('\n\n')}\nUser ID: ${appealCreator}\nMention: <@${appealCreator}>`,
          color: 0xff0dd3 // pyro purple
        }],
        allowedMentions: {
          roles: [staffRoleId],
        },
      ...
      if (!appealCreator) {
        console.log('appeal creator not found')
      } else {
      await discussionThread.send({
        content: `Please discuss this appeal.\n\nVote created by <@${interaction.user.id}>`,
        embeds: [{
          description: `${embedDescriptions.join('\n\n')}\nUser ID: ${appealCreator}\nMention: <@${appealCreator}>`,
          color: 0xff0dd3 // pyro purple
        }],
          ...
          if (crossCount === tickCount) {
            ...
            if (!acceptedChannel) {
              ...
              (acceptedChannel as TextChannel).send({
                content: `<@&${staffRoleId}>`, // Mentioning the appeal creator
                embeds: [{
              ...
              const modGuild = client.guilds.cache.get(modGuildId); // Assuming 'client' is your Discord bot client instance
            
              if (modGuild) {
                // Fetch the channel by its ID in the modGuild
                const modChannel = modGuild.channels.cache.get(modAcceptedChannel); // Assuming 'modAcceptedChannel' is the ID of the accepted channel in the modGuild
                if (modChannel && modChannel.isTextBased()) {
                  // Send the message to the mod channel in the separate server
                  (modChannel as TextChannel).send({
                    content: ``, // Mentioning the appeal creator
                    embeds: [{
                      description: embedContent,
                      color: 0x00ff00 // hex colour for green
                    }]
                  ...
                  console.log("The mod channel is invalid or not a text channel.");
                }
              } else {
                console.log("The mod guild is invalid.");
              }
            }
